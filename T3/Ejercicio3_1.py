...
# Curva paramétrica
t = np.linspace(-10.0, 10.0, num=61)  # Ajuste de intervalo
xc = np.power(t, 2) + 3 * t
yc = np.power(t, 2) + 1
zc = 3 * t + 4
...
# Derivada r'(t) y vector tangente en t = 1
r_prime_t0 = np.array([5, 2, 3])  # Derivada en t = 1
magnitude_r_prime_t0 = np.sqrt(5**2 + 2**2 + 3**2)
unit_tangent = r_prime_t0 / magnitude_r_prime_t0

# Ecuación de la línea tangente: r(t) + t * T(t)
t_line_values = np.linspace(-20.0, 20.0, num=10)
